...
def call_gemini(prompt, temperature=0.6, is_json=False):
    try:
        ...
        if response.parts:
            ...
            if response.candidates: print(f"⚠️ API가 빈 응답을 반환했습니다. (Finish Reason: {response.candidates[0].finish_reason})")
            else: print("⚠️ API가 후보(candidate) 없이 빈 응답을 반환했습니다.")
            return None
    except Exception as e:
...
def generate_impactful_titles(keyword, article_summary):
    ...
    default_title = f"{keyword}, A to Z 완벽 총정리"
    if not response_text:
        print("⚠️ 제목 생성 실패, 기본 제목을 사용합니다.")
        return [default_title]

    try:
        titles = json.loads(response_text)
        return titles if isinstance(titles, list) and titles else [default_title]
    except Exception as e:
...
def generate_meta_description(html_body_text, keyword):
...
def generate_json_ld_faq(html_body_text):
    """(분업 3) 본문 텍스트를 기반으로 JSON-LD FAQ 스키마 생성"""
    print("  ▶ (분업 3) Gemini로 JSON-LD FAQ 생성 중...")
    prompt = f"다음 글 내용을 바탕으로 FAQ 3~4개를 만들어줘. 출력은 반드시 순수한 JSON 객체 `{{...}}` 형식이어야 해. `<script>` 태그나 다른 설명은 절대 포함하지 마.\n\n[본문 내용]\n{html_body_text[:2000]}"
...
def life_tips_start(article, keyword):
    """초안을 받아 제목 생성, 콘텐츠 분업 생성, 최종 조립 및 발행을 총괄"""
    if not wp:
        print("❌ WordPress 클라이언트가 설정되지 않아 포스팅을 중단합니다.")
        return

    # 1. 매력적인 제목 생성 (기존과 동일)
    article_summary = article[:400]
    title_options = generate_impactful_titles(keyword, article_summary)
    final_title = title_options[0]
    print(f"👑 선택된 최종 제목: {final_title}")

    # 2. 이미지 생성 및 업로드 (기존과 동일)
    ...
    # ✨ 3. [신규] 분업 및 조립 프로세스
    # (분업) 각 부분을 전문 함수에 맡겨 생성
    main_body_html = generate_main_body_html(article, keyword)
    soup = BeautifulSoup(main_body_html, 'html.parser')
    body_text_for_summary = soup.get_text(strip=True, separator=' ')

    meta_description = generate_meta_description(body_text_for_summary, keyword)
    json_ld_content = generate_json_ld_faq(body_text_for_summary)

    # (파이썬 역할) 생성된 조각들을 안정적으로 처리
    toc_html = create_table_of_contents(soup)
    soup = clean_and_refine_html(soup)
    json_ld_script = f'<script type="application/ld+json">\n{json_ld_content}\n</script>' if json_ld_content and json_ld_content.strip().startswith(
        '{') else ""

    # (조립) 파이썬 코드가 모든 조각을 최종 HTML로 조립
    print("▶ (최종 조립) 모든 요소를 하나의 HTML로 결합합니다.")
...
"""
    ...
    post.custom_fields = [{'key': 'rank_math_description', 'value': meta_description},
